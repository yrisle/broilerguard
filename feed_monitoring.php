<?php
// feed_monitoring.php - Feed Monitoring Module with Database (PDO)
session_start();
...
// ===== FETCH FEED INVENTORY (current level) =====
global $pdo;
$invStmt = $pdo->prepare("SELECT current_level, capacity FROM feed_inventory WHERE user_id = ?");
$invStmt->execute([$userId]);
$inv = $invStmt->fetch(PDO::FETCH_ASSOC);
if (!$inv) {
    $inv = ['current_level' => 100.0, 'capacity' => 200.0];
}
$currentFeedLevel = (float)$inv['current_level'];
$capacity = (float)$inv['capacity'];
$feedStatus = 'normal';
if ($currentFeedLevel < 60) $feedStatus = 'danger';
elseif ($currentFeedLevel < 80) $feedStatus = 'warning';

// ===== FETCH TRANSACTIONS =====
$transStmt = $pdo->prepare("SELECT * FROM feed_transactions WHERE user_id = ? ORDER BY timestamp DESC");
...
if ($filter === '24h') {
    for ($i = 6; $i <= 22; $i += 2) {
        ...
        foreach ($filteredTransactions as $t) {
            if ($t['type'] === 'consumption' && date('H', strtotime($t['timestamp'])) >= $i && date('H', strtotime($t['timestamp'])) < $i+2) {
                $sum += $t['amount'];
            }
        }
        $consumptionData[] = round($sum, 1);
        $remainingData[] = round($currentFeedLevel - $sum, 1);
    }
} else { // 7d, 30d, custom
    ...
    for ($i = $days-1; $i >= 0; $i--) {
        ...
        $sum = 0;
        foreach ($filteredTransactions as $t) {
            if ($t['type'] === 'consumption' && date('Y-m-d', strtotime($t['timestamp'])) === $date) {
                $sum += $t['amount'];
            }
        }
        $consumptionData[] = round($sum, 1);
...
foreach ($allTransactions as $t) {
    if ($t['type'] === 'consumption') {
        if (date('Y-m-d', strtotime($t['timestamp'])) === date('Y-m-d')) $todayConsumption += $t['amount'];
        if (strtotime($t['timestamp']) >= strtotime('-7 days')) $weekConsumption += $t['amount'];
        if (strtotime($t['timestamp']) >= strtotime('-30 days')) $monthConsumption += $t['amount'];
    }
}
$avgConsumption = count($consumptionData) > 0 ? round(array_sum($consumptionData) / count($consumptionData), 1) : 0;
$minConsumption = count($consumptionData) > 0 ? min($consumptionData) : 0;
$maxConsumption = count($consumptionData) > 0 ? max($consumptionData) : 0;
$totalConsumption = array_sum($consumptionData);

// ===== FEED SCHEDULE (mock) =====
$feedSchedule = [
    ['time' => '06:00 AM', 'amount' => '5 kg', 'status' => 'completed'],
    ['time' => '10:00 AM', 'amount' => '5 kg', 'status' => 'completed'],
    ['time' => '02:00 PM', 'amount' => '5 kg', 'status' => 'pending'],
    ['time' => '06:00 PM', 'amount' => '5 kg', 'status' => 'pending'],
];

// ===== ALERTS =====
$alerts = [];
if ($currentFeedLevel < 60) $alerts[] = ['type' => 'critical', 'message' => "Critical feed level: {$currentFeedLevel}% remaining. Immediate refill required!", 'time' => 'Just now', 'icon' => 'fa-exclamation-triangle'];
elseif ($currentFeedLevel < 80) $alerts[] = ['type' => 'warning', 'message' => "Low feed warning: {$currentFeedLevel}% remaining. Please refill soon.", 'time' => 'Just now', 'icon' => 'fa-utensils'];
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .reading-card.feed-card { border-top: 4px solid var(--accent); }
        .reading-card .reading-icon { font-size: 3rem; margin-bottom: 1rem; }
        .reading-card.feed-card .reading-icon { color: var(--accent); }
        .reading-card .reading-value { font-size: 4rem; font-weight: 800; line-height: 1; margin-bottom: 0.5rem; }
        .reading-card.feed-card .reading-value { color: var(--accent); }
        ...
        .circular-progress .progress-text { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-weight: 700; font-size: 1.5rem; }
        ...
        .filter-btn { padding: 0.4rem 0.8rem; border-radius: 20px; border: 1px solid rgba(141,180,142,0.2); background: var(--bg-card); cursor: pointer; font-size: 0.8rem; font-weight: 500; color: var(--text-secondary); transition: all 0.2s; font-family: 'Inter', sans-serif; white-space: nowrap; }
        .filter-btn.active { background: var(--accent); color: #FFFFFF; border-color: var(--accent); font-weight: 600; }
        .filter-separator { width: 1px; height: 24px; background: rgba(141,180,142,0.3); margin: 0 0.5rem; }
        .date-input { padding: 0.35rem 0.7rem; border-radius: 20px; border: 1px solid rgba(141,180,142,0.2); font-family: 'Inter', sans-serif; font-size: 0.8rem; }
        ...
        .schedule-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.8rem; }
        .schedule-item { background: var(--bg-card); border-radius: 10px; padding: 1rem; text-align: center; border: 1px solid rgba(141,180,142,0.10); }
        .schedule-item .schedule-time { font-weight: 700; font-size: 0.9rem; }
        .schedule-item .schedule-amount { font-size: 0.8rem; color: var(--text-muted); margin-top: 0.3rem; }
        .schedule-item .schedule-status { margin-top: 0.5rem; font-size: 0.7rem; }
...
<div class="main-content" id="mainContent">
    ...
    <div class="page-content">
        <h1 class="page-title"><i class="fas fa-utensils" style="color:var(--accent);"></i> Feed Monitoring</h1>
        <p class="page-subtitle">Real-time feed level monitoring and consumption tracking</p>
            ...
            <div class="reading-card feed-card">
                ...
                <div class="circular-progress">
                    <svg width="120" height="120" viewBox="0 0 120 120">
                        ...
                        <circle cx="60" cy="60" r="50" fill="none" stroke="#4D724D" stroke-width="8"
                            stroke-dasharray="<?php echo ($currentFeedLevel / 100) * 314; ?> 314"
                            stroke-linecap="round"/>
                    </svg>
                    <div class="progress-text"><?php echo round($currentFeedLevel); ?>%</div>
                </div>
                <div class="reading-label" style="margin-top:1rem;">Current Feed Level</div>
                <span class="reading-status status-<?php echo $feedStatus; ?>"><?php echo ucfirst($feedStatus); ?></span>
            </div>
            <div class="reading-card" style="border-top: 4px solid var(--green);">
                <div class="reading-icon" style="color:var(--green);"><i class="fas fa-chart-bar"></i></div>
                <div style="display:flex;flex-direction:column;gap:0.8rem;margin-top:1rem;">
                    <div style="display:flex;justify-content:space-between;padding:0.5rem 1rem;background:var(--bg-secondary);border-radius:8px;">
                        <span>Today</span><strong><?php echo number_format($todayConsumption, 1); ?> kg</strong>
                    </div>
                    <div style="display:flex;justify-content:space-between;padding:0.5rem 1rem;background:var(--bg-secondary);border-radius:8px;">
                        <span>This Week</span><strong><?php echo number_format($weekConsumption, 1); ?> kg</strong>
                    </div>
                    <div style="display:flex;justify-content:space-between;padding:0.5rem 1rem;background:var(--bg-secondary);border-radius:8px;">
                        <span>This Month</span><strong><?php echo number_format($monthConsumption, 1); ?> kg</strong>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="stats-mini-grid">
            <div class="stat-mini-card"><div class="stat-mini-value"><?php echo number_format($totalConsumption, 1); ?> kg</div><div class="stat-mini-label">Total Consumption</div></div>
            <div class="stat-mini-card"><div class="stat-mini-value"><?php echo number_format($avgConsumption, 1); ?> kg</div><div class="stat-mini-label">Average Daily</div></div>
            <div class="stat-mini-card"><div class="stat-mini-value"><?php echo number_format($minConsumption, 1); ?> kg</div><div class="stat-mini-label">Min Consumption</div></div>
            <div class="stat-mini-card"><div class="stat-mini-value"><?php echo number_format($maxConsumption, 1); ?> kg</div><div class="stat-mini-label">Max Consumption</div></div>
        </div>
        ...
        <div class="charts-grid">
            <div class="chart-card">
                <div class="chart-card-header"><span class="chart-card-title"><i class="fas fa-chart-line" style="color:var(--accent);"></i> Feed Consumption</span><span class="card-badge badge-info">kg</span></div>
                <div class="chart-wrapper"><canvas id="feedConsumptionChart"></canvas></div>
            </div>
            <div class="chart-card">
                <div class="chart-card-header"><span class="chart-card-title"><i class="fas fa-chart-area" style="color:var(--green);"></i> Feed Level Remaining</span><span class="card-badge badge-info">%</span></div>
        ...
        <div class="chart-card" style="margin-bottom:1.5rem;">
            <div class="chart-card-header"><span class="chart-card-title"><i class="fas fa-calendar-check"></i> Feeding Schedule</span></div>
            <div class="schedule-grid">
                <?php foreach ($feedSchedule as $schedule): ?>
        ...
        <div class="chart-card" style="padding:0;overflow:hidden;">
            <div class="chart-card-header" style="padding:1.5rem 1.5rem 0 1.5rem;">
                <span class="chart-card-title"><i class="fas fa-list"></i> Feed Transactions</span>
            ...
            <div class="table-container" style="padding:0 1.5rem 1.5rem 1.5rem;">
                <table>
                    <thead><tr><th>Date & Time</th><th>Type</th><th>Amount</th><th>Source</th><th>Notes</th><th>Remaining</th></tr></thead>
                    <tbody>
                        <?php if (empty($filteredTransactions)): ?>
                        <tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--text-muted);">No transactions found matching your filters.</td></tr>
                        <?php else: ?>
                            <?php foreach ($filteredTransactions as $t): ?>
                            <tr>
                                <td><?php echo date('M d, h:i A', strtotime($t['timestamp'])); ?></td>
                                <td><span class="card-badge <?php echo $t['type'] === 'refill' ? 'badge-success' : 'badge-info'; ?>"><?php echo ucfirst($t['type']); ?></span></td>
                                <td><strong><?php echo number_format($t['amount'], 1); ?> kg</strong></td>
                                <td><span class="card-badge badge-warning"><?php echo ucfirst($t['source'] ?? 'manual'); ?></span></td>
                                <td style="color:var(--text-muted);font-size:0.8rem;"><?php echo htmlspecialchars($t['notes'] ?? '—'); ?></td>
                                <td><?php echo isset($t['remaining']) ? number_format($t['remaining'], 1) . ' kg' : '—'; ?></td>
                            </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
...
<script>
    ...
    const consumptionData = <?php echo json_encode($consumptionData); ?>;
    ...
    function initCharts() {
        ...
        [feedConsumptionChart, feedLevelChart].forEach(chart => { if (chart) chart.destroy(); });

        const consumptionCtx = document.getElementById('feedConsumptionChart');
        if (consumptionCtx) {
            feedConsumptionChart = new Chart(consumptionCtx, {
                type: 'bar',
                data: { labels: chartLabels, datasets: [{ label: 'Feed Consumed (kg)', data: consumptionData, backgroundColor: '#4D724D', borderRadius: 8, maxBarThickness: 40 }] },
        ...
        if (levelCtx) {
            feedLevelChart = new Chart(levelCtx, {
                type: 'line',
                data: { labels: chartLabels, datasets: [{ label: 'Feed Level (%)', data: remainingData.map(v => (v / <?php echo $capacity; ?>) * 100), borderColor: '#8DB48E', backgroundColor: 'rgba(141,180,142,0.1)', fill: true, tension: 0.4, pointRadius: 3, borderWidth: 2 }] },
    ...
    function refreshWeather() { window.location.href = 'feed_monitoring.php?refresh_weather=1'; }
    ...
    window.addEventListener('resize', function() { if (feedConsumptionChart) feedConsumptionChart.resize(); if (feedLevelChart) feedLevelChart.resize(); });
</script>
</body>
</html>
