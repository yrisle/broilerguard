...
// Initialize feed transactions if not exists
if (!isset($_SESSION['feed_transactions'])) {
    $_SESSION['feed_transactions'] = [];
}

// Function to deduct feed (called by feed_dispenser.php)
function deductFeed($amount, $source = 'auto_dispenser', $notes = '') {
    if (!isset($_SESSION['feed_inventory'])) {
        return ['success' => false, 'message' => 'Inventory not initialized'];
    }
    
    if ($_SESSION['feed_inventory']['current_level'] - $amount < 0) {
        return ['success' => false, 'message' => 'Insufficient feed level'];
    }
    
    $_SESSION['feed_inventory']['current_level'] -= $amount;
    ...
    array_unshift($_SESSION['feed_transactions'], $transaction);
    
    return ['success' => true, 'message' => 'Feed deducted', 'remaining' => $_SESSION['feed_inventory']['current_level']];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    if ($action === 'update_settings') {
        $_SESSION['feed_inventory']['capacity'] = floatval($_POST['capacity'] ?? 200);
        $_SESSION['feed_inventory']['alert_threshold'] = floatval($_POST['alert_threshold'] ?? 20);
        $_SESSION['feed_inventory']['critical_threshold'] = floatval($_POST['critical_threshold'] ?? 10);
        $_SESSION['feed_inventory']['supplier'] = $_POST['supplier'] ?? 'Local Feed Supply';
        $_SESSION['feed_inventory']['feed_type'] = $_POST['feed_type'] ?? 'Broiler Starter';
        ...
        $capacity = $_SESSION['feed_inventory']['capacity'];
        $newLevel = min($capacity, $_SESSION['feed_inventory']['current_level'] + $amount);
        $_SESSION['feed_inventory']['current_level'] = $newLevel;
        ...
        $response = ['success' => true, 'message' => "Added {$amount} kg of feed", 'new_level' => $newLevel];
        ...
        $result = deductFeed($amount, 'manual', $notes);
        $response = $result;
        
    } elseif ($action === 'delete_transaction') {
        $transactionId = $_POST['id'] ?? '';
        foreach ($_SESSION['feed_transactions'] as $key => $trans) {
            if ($trans['id'] == $transactionId) {
                if ($trans['type'] === 'refill') {
                    $_SESSION['feed_inventory']['current_level'] -= $trans['amount'];
                } elseif ($trans['type'] === 'consumption') {
                    $_SESSION['feed_inventory']['current_level'] += $trans['amount'];
                }
                unset($_SESSION['feed_transactions'][$key]);
                $_SESSION['feed_transactions'] = array_values($_SESSION['feed_transactions']);
        ...
        $response = [
            'success' => true,
            'current_level' => $_SESSION['feed_inventory']['current_level'],
            'capacity' => $_SESSION['feed_inventory']['capacity'],
            'percentage' => ($_SESSION['feed_inventory']['current_level'] / $_SESSION['feed_inventory']['capacity']) * 100,
            'alert_threshold' => $_SESSION['feed_inventory']['alert_threshold'],
            'critical_threshold' => $_SESSION['feed_inventory']['critical_threshold']
        ];
    }
...
$inventory = $_SESSION['feed_inventory'];
$transactions = $_SESSION['feed_transactions'];
$currentLevel = isset($inventory['current_level']) ? $inventory['current_level'] : 100;
$capacity = isset($inventory['capacity']) ? $inventory['capacity'] : 200;
$percentage = ($capacity > 0) ? ($currentLevel / $capacity) * 100 : 0;
$alertThreshold = isset($inventory['alert_threshold']) ? $inventory['alert_threshold'] : 20;
$criticalThreshold = isset($inventory['critical_threshold']) ? $inventory['critical_threshold'] : 10;
$isLow = $currentLevel <= $alertThreshold;
$isCritical = $currentLevel <= $criticalThreshold;
$supplier = isset($inventory['supplier']) ? $inventory['supplier'] : 'Local Feed Supply';
$feedType = isset($inventory['feed_type']) ? $inventory['feed_type'] : 'Broiler Starter';
$lastRefill = isset($inventory['last_refill']) ? $inventory['last_refill'] : date('Y-m-d H:i:s');
    ...
    foreach ($transactions as $trans) {
        if (isset($trans['timestamp'])) {
            ...
            if ($trans['type'] === 'consumption') {
                ...
                if ($transDate >= $monthAgo) $monthlyConsumption += $trans['amount'];
                
                if (isset($trans['source'])) {
                    if ($trans['source'] === 'auto_dispenser') $autoDispensed += $trans['amount'];
                    if ($trans['source'] === 'manual') $manualDispensed += $trans['amount'];
                }
            }
            
            if ($trans['type'] === 'refill') {
                $totalRefills += $trans['amount'];
                if (isset($trans['cost'])) $totalCost += $trans['cost'];
            }
        }
    }
}
...
<div class="main-content" id="mainContent">
    ...
    <div class="page-content">
        ...
        <div class="stats-grid">
            <div class="stat-card"><div class="stat-info"><div class="stat-value" style="color:#2E7D32;"><?php echo number_format($currentLevel, 1); ?> kg</div><div class="stat-label">Current Level</div></div><div class="stat-icon"><i class="fas fa-box"></i></div></div>
            <div class="stat-card"><div class="stat-info"><div class="stat-value" style="color:#D4A017;"><?php echo number_format($todayConsumption, 1); ?> kg</div><div class="stat-label">Today's Consumption</div></div><div class="stat-icon"><i class="fas fa-chart-line"></i></div></div>
            <div class="stat-card"><div class="stat-info"><div class="stat-value" style="color:#1565C0;"><?php echo number_format($weeklyConsumption, 1); ?> kg</div><div class="stat-label">This Week</div></div><div class="stat-icon"><i class="fas fa-calendar-week"></i></div></div>
            <div class="stat-card"><div class="stat-info"><div class="stat-value" style="color:#7B1FA2;"><?php echo number_format($autoDispensed, 1); ?> kg</div><div class="stat-label">Auto Dispensed</div></div><div class="stat-icon"><i class="fas fa-robot"></i></div></div>
                ...
                <table class="transactions-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge-<?php echo $trans['type'] ?? 'refill'; ?>">
                                    <?php echo ($trans['type'] ?? 'refill') === 'refill' ? '➕ Refill' : '🍽️ Consumption'; ?>
                                </span>
                            </td>
                            <td style="font-weight: 500;"><?php echo number_format($trans['amount'] ?? 0, 1); ?> kg</td>
                            <td>
                                <?php if (isset($trans['source'])): ?>
                                    <span class="badge-<?php echo $trans['source'] === 'auto_dispenser' ? 'auto' : 'manual'; ?>">
                                        <?php echo $trans['source'] === 'auto_dispenser' ? '🤖 Auto Dispenser' : '✋ Manual'; ?>
                                    </span>
                                <?php else: ?>
                                    <span>—</span>
                                <?php endif; ?>
...
<script>
    ...
    async function confirmRefill() {
        ...
        try {
            var response = await fetch('feed_inventory.php', {
                method: 'POST',
    ...
    async function confirmDeduct() {
        ...
        try {
            ...
            showToast('Error deducting feed', true);
        }
        closeDeductModal();
    }
    ...
    async function checkInventoryStatus() {
        try {
            ...
            var data = await response.json();
            if (data.success) {
                showToast('Current level: ' + data.current_level + ' kg (' + data.percentage.toFixed(1) + '%)', false);
            }
        } catch(error) {
